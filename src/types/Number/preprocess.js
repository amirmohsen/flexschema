...
export const validateMinAndMax = ({schema}) => {
	...
	else if(type(schema.inclusiveMin) !== 'boolean') {
	...
	else if(type(schema.inclusiveMax) !== 'boolean') {
		throw new FlexSchemaError({
...
export const validateNumberType = ({schema}) => {
	...
	else if(type(schema.numberType) !== 'string' || !numberTypes.includes(schema.numberType)) {
		throw new FlexSchemaError({
			message: `Number Schema's optional field "numberType" has to be a string and one of "integer", "float", "digit", "natural", "whole", "real", "fractional"`,
			code: FlexSchemaError.CODES.BAD_SCHEMA,
			details: {
				schema
			}
